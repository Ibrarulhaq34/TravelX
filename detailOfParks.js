...
const DetailOfParks = ({ navigation, route }) => {
  ...
  const getDatabase = async () => {
    try {
      const data = await firestore().collection('Park').doc('MUjayHETj7DJoTllQx9I').get();
  ...
  return (
    <View>
      <ScrollView style={styles.listView}>
        ...
        ) : route.params?.name === 'Allama Iqbal Park' ? (
          <>
            <Image
        ...
        ) : route.params?.name === 'Eyeview park' ? (
          <>
            <Image
